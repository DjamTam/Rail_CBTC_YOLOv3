...
def main():
    ...
    api_thread = threading.Thread(target=lambda: app.run(host=cfg.API_HOST, port=cfg.API_PORT, debug=False), daemon=True)
    api_thread.start()
    ...
    while True:
        ...
        shared_status["decision"] = dec
        shared_status["persons_in_roi"] = persons_in_roi
        shared_status["max_confidence"] = max_conf
        shared_status["fps_avg"] = kpi["fps_avg"]

        if dec in ("STOP_REQUEST", "UNCERTAIN"):
            logger.log({
                "event": dec,
                "persons_in_roi": persons_in_roi,
